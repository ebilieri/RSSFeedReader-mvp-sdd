...
- [ ] T018 [US1] Criar `frontend/RSSFeedReader.UI/Pages/Subscriptions.razor` com diretiva `@page "/"`, injetar `SubscriptionApiService`, declarar variáveis de estado: `newUrl`, `subscriptions` (List<SubscriptionItemDto>), `successMessage`, `errorMessage`, `isLoading`
- [ ] T019 [US1] Implementar formulário em `Subscriptions.razor`: campo `<input>` com `@bind="newUrl"`, botão "Adicionar" com handler `OnAddSubmit` — handler valida campo vazio (FR-008), chama `AddAsync`, adiciona item retornado à lista local, limpa `newUrl` (FR-006), exibe `successMessage` por 3s via `Task.Delay` + `StateHasChanged` (FR-007)
- [ ] T020 [US1] Adicionar `OnInitializedAsync` em `Subscriptions.razor` para carregar lista inicial via `GetAllAsync()` ao abrir a página
